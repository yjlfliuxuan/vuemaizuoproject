...
router.get('/list', function (req, res) {
  ...
  MongoClient.connect(url, { useNewUrlParser: true }, function (err, client) {
    if (err) {
      ...
      var db = client.db('vueproject');

      async.waterfall([
        function (cb) {
          db.collection('maizuo').find(param).count(function (err, num) {
            if (err) {
              cb(err);
            } else {
              cb(null, num);
        ...
        function (num, cb) {
          db.collection('maizuo').find(param).skip(pageSize * pageNum - pageSize).limit(pageSize).toArray(function (err, data) {
            if (err) {
              cb(err);
            } else {
